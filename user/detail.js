...
function renderEpisodeButtons(serverData, movieSlug, currentServerIdx) {
    ...
    // 👇 ÉP XUNG CSS: Tiêu diệt lệnh Grid !important cũ trên Mobile
    epListDiv.style.setProperty('display', 'block', 'important');
    epListDiv.style.setProperty('width', '100%', 'important');
    ...
    if (serverData.length <= CHUNK_SIZE) {
        ...
        let rangeHTML = `<div class="ep-tabs-wrapper" style="display: flex; flex-wrap: nowrap; gap: 8px; margin-bottom: 5px; overflow-x: auto; padding-bottom: 10px; width: 100%;">`;
...
function renderChunk(startIndex, endIndex, movieSlug, currentServerIdx, container) {
    // 👇 ÉP CSS GRID VÀO KHUNG CHỨA NÚT TẬP PHIM
    container.style.setProperty('display', 'grid', 'important');
    // Máy to sẽ chia nhiều ô, điện thoại nhỏ tự ép về các ô rộng 70px (khoảng 4-5 ô/hàng)
    container.style.setProperty('grid-template-columns', 'repeat(auto-fill, minmax(70px, 1fr))', 'important');
